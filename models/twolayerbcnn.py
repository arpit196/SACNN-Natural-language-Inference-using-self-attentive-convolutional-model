...
class Attention2lyrCnn(BaseSiameseNet):
    ...
    def siamese_layer(self, sequence_len, model_cfg):
        ...
        with tf.name_scope('convolutional_layer'):
            ...
            self._X1_conv = tf.layers.dropout(X1_conv_1, rate=self.dropout, training=self.is_training)
            self._X2_conv = tf.layers.dropout(X2_conv_1, rate=self.dropout, training=self.is_training)
        ...
        with tf.name_scope('comparison_layer'):
            X1_comp = tf.layers.dense(
                tf.concat([self._X1_conv, self._beta, self._beta1,self._X2_conv2,self._beta_lr2,self._beta1_lr2], 2),
