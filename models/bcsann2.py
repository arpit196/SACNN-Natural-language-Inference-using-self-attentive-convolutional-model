    ...
    def siamese_layer(self, sequence_len, model_cfg):
        ...
        with tf.name_scope('comparison_layer'):
            ...
            X1_comp1 = tf.layers.dense(
                tf.concat([X1_comp, self._beta1, tf.add(X1_comp,self._beta1),tf.tensordot(self._beta1,X1_comp)], 2),
                _comparison_output_size,
                activation=tf.nn.relu,
                name='comparison_nn'
            )
            ...
            X2_comp = tf.layers.dense(
                tf.concat([self._X2_conv, self._alpha, tf.add(self._X2_conv,self._alpha), tf.tensordot(self._X2_conv,self._alpha)], 2),
                _comparison_output_size,
                activation=tf.nn.relu,
                name='comparison_nn',
                reuse=True
            ...
            X2_comp1 = tf.layers.dense(
                tf.concat([X2_comp, self._alpha1, tf.add(X2_comp,self._alpha1), tf.tensordot(self._alpha1,X2_comp)], 2),
                _comparison_output_size,
                activation=tf.nn.relu,
                name='comparison_nn'
            )
