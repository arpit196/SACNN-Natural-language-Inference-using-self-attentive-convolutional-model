...
class SNLIDataset(DatasetExperiment):
    ...
    def __init__(self, *args):
        super().__init__(*args)
        dataset_train = pd.read_csv('{}{}'.format(self.data_dir, 'snli_1.0_train.txt'),
        ...
        self.num_test = len(dataset_test)
        self.train = dataset_train
        self.dev = dataset_dev
    ...
    def train_labels(self):
        return self.train[ColumnType.gold_label.name].as_matrix()
    ...
    def dev_labels(self):
        return self.dev[ColumnType.gold_label.name].as_matrix()
...
class SICKDataset(DatasetExperiment):
    ...
    def train_set_pairs(self):
        return self.train[['sentence_A', 'sentence_B']].as_matrix()

    def train_labels(self):
        return self.train['entailment_label'].as_matrix()

    def dev_set(self):
        return self.dev

    def dev_set_pairs(self):
        return self.dev[['sentence_A', 'sentence_B']].as_matrix()

    def dev_labels(self):
        return self.dev['entailment_label'].as_matrix()

    def test_set(self):
        return self.test

    def test_set_pairs(self):
        return self.test[['sentence_A', 'sentence_B']].as_matrix()

    def test_labels(self):
        return self.test['entailment_label'].as_matrix()
